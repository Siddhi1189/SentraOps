...
export function PublicStatusIncidentsPage() {
  const { orgSlug = '' } = useParams<{ orgSlug: string }>();
  const { data: incidentsRes, isLoading, isError, error, refetch } = usePublicStatusIncidentsQuery(orgSlug);
  ...
  useEffect(() => {
    ...
    document.title = `${orgName} — Incident History`;
  }, [settings, orgSlug]);
  ...
  if (isError) {
    return (
      <PublicStatusLayout orgSlug={orgSlug} settings={settings}>
        ...
        >
          <h3 style={{ margin: '0 0 8px 0', fontSize: '1.125rem' }}>Unable to Load Incident History</h3>
          <p style={{ margin: '0 0 16px 0', color: 'var(--color-text-muted, #64748b)', fontSize: '0.875rem' }}>
            {apiError?.error?.message || 'Failed to retrieve incident history.'}
          </p>
          <button
            type="button"
            onClick={() => refetch()}
